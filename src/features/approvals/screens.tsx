"use client";
import { DetailTabs } from "../identity/detail-tabs";
import { relatedTabs } from "../relationships/related-records";
import type { RelatedGroup } from "../relationships/repository";
import { DateValue, useI18n } from "@/i18n/provider";
import Link from "next/link";
import { PageHeading } from "@/components/ui/page-heading";
import { Breadcrumbs } from "@/components/ui/breadcrumbs";
import { Badge, type Tone } from "@/components/ui/badge";
import { BrowseTable } from "../identity/browse-table";
import { DemoNotice, Details, Summary } from "../identity/shared";
import type {
  ApprovalTemplate,
  ApprovalRecord,
  ApprovalStatus,
  RequestType,
} from "./types";
const types: Record<RequestType, string> = {
  issue: "발급",
  renew: "갱신",
  revoke: "폐기",
};
const states: Record<ApprovalStatus, { label: string; tone: Tone }> = {
  pending: { label: "결재 대기", tone: "warning" },
  approved: { label: "승인", tone: "success" },
  rejected: { label: "반려", tone: "danger" },
};
const typeOptions = Object.entries(types).map(([value, label]) => ({
  value,
  label,
}));
const link = (route: string, id: string, label: string) => (
  <Link className="identity-link" href={`${route}/${id}`}>
    {label}
  </Link>
);
function timestamp(value: string | null) {
  return <DateValue value={value} time empty="미처리" />;
}
function TemplateStatus({ status }: { status: ApprovalTemplate["status"] }) {
  const { t } = useI18n();
  return (
    <Badge tone={status === "active" ? "success" : "neutral"}>
      {status === "active" ? t("사용 중") : t("사용 중지")}
    </Badge>
  );
}
function ApprovalBadge({ status }: { status: ApprovalStatus }) {
  return <Badge tone={states[status].tone}>{states[status].label}</Badge>;
}
export function TemplatesList({ rows }: { rows: ApprovalTemplate[] }) {
  const { t } = useI18n();
  return (
    <>
      <PageHeading
        title={t("결재 템플릿")}
        description={t(
          "요청 유형별 결재 템플릿의 기본 정보와 작성 안내를 조회합니다.",
        )}
      />
      <DemoNotice />
      <Summary
        items={[
          { label: t("전체 템플릿"), value: rows.length },
          {
            label: t("사용 중"),
            value: rows.filter((t) => t.status === "active").length,
          },
        ]}
      />
      <BrowseTable
        title={t("결재 템플릿 목록")}
        rows={rows}
        searchText={(r) =>
          `${r.id} ${r.name} ${r.description} ${r.instructions}`
        }
        sortValue={(r, k) => (k === "updatedAt" ? r.updatedAt : r.name)}
        filters={[
          {
            key: "status",
            label: t("템플릿 상태 필터"),
            options: [
              { value: "active", label: t("사용 중") },
              { value: "inactive", label: t("사용 중지") },
            ],
            matches: (r, v) => r.status === v,
          },
          {
            key: "type",
            label: t("요청 유형 필터"),
            options: typeOptions,
            matches: (r, v) => r.type === v,
          },
        ]}
        columns={[
          {
            key: "name",
            header: t("이름"),
            sortable: true,
            render: (r) => (
              <>
                {link("/approval-templates", r.id, r.name)}
                <div className="identity-meta">{r.id}</div>
              </>
            ),
          },
          {
            key: "description",
            header: t("설명"),
            render: (r) => r.description,
          },
          {
            key: "type",
            header: t("요청 유형"),
            render: (r) => t(types[r.type]),
          },
          { key: "version", header: t("버전"), render: (r) => `v${r.version}` },
          {
            key: "status",
            header: t("상태"),
            render: (r) => <TemplateStatus status={r.status} />,
          },
          {
            key: "updatedAt",
            header: t("수정일"),
            sortable: true,
            render: (r) => timestamp(r.updatedAt),
          },
        ]}
      />
    </>
  );
}
export function TemplateScreen({
  data: template,
  groups = [],
}: {
  data: ApprovalTemplate;
  groups?: RelatedGroup[];
}) {
  const { t } = useI18n();
  return (
    <>
      <Breadcrumbs
        items={[
          { label: t("결재 템플릿"), href: "/approval-templates" },
          { label: template.name },
        ]}
      />
      <PageHeading title={template.name} description={template.description} />
      <DemoNotice />
      <DetailTabs
        items={[
          {
            value: "info",
            label: t("기본 정보"),
            content: (
              <section className="ui-panel">
                <h2>{t("결재 템플릿 정보")}</h2>
                <Details
                  items={[
                    { label: t("템플릿 ID"), value: template.id },
                    { label: t("이름"), value: template.name },
                    { label: t("설명"), value: template.description },
                    { label: t("요청 유형"), value: t(types[template.type]) },
                    { label: t("버전"), value: `v${template.version}` },
                    {
                      label: t("상태"),
                      value: <TemplateStatus status={template.status} />,
                    },
                    { label: t("작성 안내"), value: template.instructions },
                    {
                      label: t("등록일"),
                      value: timestamp(template.createdAt),
                    },
                    {
                      label: t("수정일"),
                      value: timestamp(template.updatedAt),
                    },
                  ]}
                />
              </section>
            ),
          },
          ...relatedTabs(groups, t),
        ]}
      />
    </>
  );
}
export function ApprovalsList({ rows }: { rows: ApprovalRecord[] }) {
  const { t } = useI18n();
  const templates = Array.from(
    new Map(
      rows.map((r) => [
        r.templateId,
        { id: r.templateId, name: r.templateName },
      ]),
    ).values(),
  );
  return (
    <>
      <PageHeading
        title={t("결재")}
        description={t("결재 요청의 상세 정보와 처리 결과를 조회합니다.")}
      />
      <DemoNotice />
      <Summary
        items={[
          { label: t("전체 결재"), value: rows.length },
          {
            label: t("결재 대기"),
            value: rows.filter((r) => r.status === "pending").length,
          },
          {
            label: t("승인"),
            value: rows.filter((r) => r.status === "approved").length,
          },
          {
            label: t("반려"),
            value: rows.filter((r) => r.status === "rejected").length,
          },
        ]}
      />
      <BrowseTable
        title={t("결재 목록")}
        rows={rows}
        searchText={(r) =>
          `${r.id} ${r.title} ${r.reason} ${r.comment ?? ""} ${r.requester.name} ${r.reviewer?.name ?? ""}`
        }
        sortValue={(r, k) => (k === "title" ? r.title : r.requestedAt)}
        filters={[
          {
            key: "status",
            label: t("결재 상태 필터"),
            options: Object.entries(states).map(([value, s]) => ({
              value,
              label: s.label,
            })),
            matches: (r, v) => r.status === v,
          },
          {
            key: "type",
            label: t("요청 유형 필터"),
            options: typeOptions,
            matches: (r, v) => r.type === v,
          },
          {
            key: "template",
            label: t("결재 템플릿 필터"),
            options: templates.map((t) => ({ value: t.id, label: t.name })),
            matches: (r, v) => r.templateId === v,
          },
        ]}
        columns={[
          {
            key: "title",
            header: t("결재 요청"),
            sortable: true,
            render: (r) => (
              <>
                {link("/approvals", r.id, r.title)}
                <div className="identity-meta">{r.id}</div>
              </>
            ),
          },
          {
            key: "status",
            header: t("결재 상태"),
            render: (r) => <ApprovalBadge status={r.status} />,
          },
          {
            key: "type",
            header: t("요청 유형"),
            render: (r) => t(types[r.type]),
          },
          {
            key: "requester",
            header: t("요청자"),
            render: (r) => link("/users", r.requester.id, r.requester.name),
          },
          {
            key: "reviewer",
            header: t("결재자"),
            render: (r) =>
              r.reviewer
                ? link("/users", r.reviewer.id, r.reviewer.name)
                : t("미지정"),
          },
          {
            key: "requestedAt",
            header: t("요청일"),
            sortable: true,
            render: (r) => timestamp(r.requestedAt),
          },
        ]}
      />
    </>
  );
}
export function ApprovalScreen({ data: a }: { data: ApprovalRecord }) {
  const { t } = useI18n();
  return (
    <>
      <Breadcrumbs
        items={[{ label: t("결재"), href: "/approvals" }, { label: a.id }]}
      />
      <PageHeading
        title={a.title}
        description={t("결재 요청 정보와 처리 결과를 확인합니다.")}
      />
      <DemoNotice />
      <section className="ui-panel">
        <h2>{t("결재 정보")}</h2>
        <Details
          items={[
            { label: t("결재 ID"), value: a.id },
            { label: t("요청 유형"), value: t(types[a.type]) },
            {
              label: t("결재 상태"),
              value: <ApprovalBadge status={a.status} />,
            },
            {
              label: t("결재 템플릿"),
              value: link(
                "/approval-templates",
                a.templateId,
                `${a.templateName} · v${a.templateVersion}`,
              ),
            },
            {
              label: t("대상 API 키"),
              value: link("/api-keys", a.keyId, a.keyName),
            },
            {
              label: t("요청자"),
              value: link("/users", a.requester.id, a.requester.name),
            },
            {
              label: t("결재자"),
              value: a.reviewer
                ? link("/users", a.reviewer.id, a.reviewer.name)
                : t("미지정"),
            },
            { label: t("요청일"), value: timestamp(a.requestedAt) },
            { label: t("처리일"), value: timestamp(a.decidedAt) },
            { label: t("요청 사유"), value: a.reason },
            {
              label: t("결재 의견"),
              value: a.comment ?? t("등록된 의견 없음"),
            },
          ]}
        />
      </section>
    </>
  );
}
