...
import { existingGraph, saveDemoGraph } from "./demo";
export type Result =
...
export async function loadAuthorization(): Promise<Result> {
  try {
    ...
    const { loadDefinitions } = await import("../definitions/actions");
    const result = await loadDefinitions();
    if (!result.data) return { error: result.error ?? "REQUEST_FAILED" };
...
export async function changeAuthorization(
  ...
  try {
    ...
    if (change.type === "resource" || change.type === "policy")
