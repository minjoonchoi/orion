...
표: 결재 문서명/ID, 요청자, 결재 상태, 후속 처리 상태, 현재 단계. 검색은 열람 가능한 문서의 제목·ID·사유·계정 문맥 내에서 수행한다. 문서명→상세. 결재 작성→APR-02의 등록 템플릿 선택. 발급 요청 전용 버튼은 표시하지 않는다. 검색 결과·전체 건수·페이지 수 모두 같은 열람자 조건을 적용한다. 필터 확장은 기획 가능하지만 현재 P18 목록은 검색·페이지 이동 중심이며 유형/상태 필터를 완료로 표시하지 않는다.

## APR-02 요청 — `/approvals/new` 및 KEY-03/04/05 진입

`/approvals`는 결재 문서와 결재 템플릿 탭을 통합한다. `/approvals/new`에서는 검색 가능한 등록 템플릿 목록에서 하나를 선택한다. 유형은 선택된 템플릿에서 결정하며 별도의 하드코딩된 유형 선택 단계를 제공하지 않는다. 등록 템플릿이 없으면 작성 시작은 비활성화한다. 교체·폐기는 활성 API 키도 선택해야 작성할 수 있다. 선택 후 작성 페이지로 이동한다. API 키 목록의 신규 발급은 동일한 요청·검토 폼을 모달로 제공한다. 제출된 결재는 스냅샷으로 유지하며 수정 기능은 제공하지 않는다.
...
APR-01의 결재 목록은 표시 제목 전체(템플릿 이름 · 서비스 어카운트), 문서 ID, 요청 사유로 검색한다. 동일 Documents/BrowseTable과 제목 생성 함수를 재사용한다. P18 후속 수정에서 구분점 `·`를 포함한 제목 검색을 보완하며 열람자 필터는 검색보다 먼저 적용한다.

엔드포인트는 이름·경로·HTTP 메서드·ID 검색, 페이지 이동, 선택한 항목만 보기를 제공한다. 검색·페이지 변경은 선택을 유지하고 관리 서비스 변경 시 선택을 초기화한다. 선택 건수와 검색 결과 건수를 구분한다.

작성기는 선택한 템플릿 유형 안에서만 템플릿을 변경할 수 있다. 존재하지 않는 template ID 직접 접근은 오류 안내를 표시하고 서버의 request 명령도 거부한다. 결재 참여 사용자·조직과 관리자 권한은 서버 데이터에서 받아 표시하며 데모 사용자 선택/모의 실행 버튼은 없다.

템플릿 목록 검색·페이지 변경 중에도 선택을 유지하고 선택된 템플릿 요약을 목록 아래에 표시한다.
