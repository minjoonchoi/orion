...
제품 UI에서 실행 사용자를 바꾸지 않는다. 현재 사용자 ID는 기존 API의 세션에서 제공하며 접근 권한을 테스트할 때는 테스트 API 픽스처로 주입한다. HTML의 샘플 세션은 `tools/demo` 어댑터로 분리하고 실제 화면의 모델 검증·열람 필터·결재선·템플릿 스냅샷 규칙을 재사용한다. HTML의 로그인은 화면 전환 시나리오이고 실제 IdP 인증이 아니다. 새로고침하면 샘플 세션이 초기화된다.
