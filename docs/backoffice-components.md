...
| 입력     | Field, Input, Textarea, Select      | label 연결, 도움말, 오류 설명, 필수·비활성 상태                  |
...
Field의 속성을 반드시 입력 요소에 전달합니다. 오류는 색상뿐 아니라 텍스트와 `aria-describedby`로 연결합니다. 일반적인 required/email 검사는 네이티브 폼 검증을 사용하고 업무 규칙은 호출부에서 추가합니다. Select는 네이티브 단일 선택입니다.
...
브라우저 자동 접근성 검사는 수동 스크린리더 점검을 대체하지 않습니다. 실제 기능 연결 시 새로운 라벨·오류 메시지·권한 상태도 확인해야 합니다.
