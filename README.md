...
리소스 정의 수정은 `/resources`에서 synced revision과 Out of sync 변경을 비교하고, 변경 내용을 검토한 뒤 명시적으로 Sync합니다. 정책은 `/policies/sync`에서 접근 가능한 리소스와 전처리·후처리 handler 변경을 같은 방식으로 검토합니다. Sync 이력은 revision 단위로 남기며 이전 revision으로 rollback할 수 있습니다. [리소스 YAML](config/resources/orion-resources.yaml), [정책 YAML](config/policies/orion-policies.yaml), [Sync·rollback API 계약](docs/resource-gitops.md)을 참고하세요. `npm run validate:resources`로 원본 YAML 스키마를 검사합니다. 예제 모드는 세션 데이터 적용이며, 운영 모드는 Orion 백엔드의 config 조회·검증·DB 트랜잭션 구현이 필요합니다. 외부 배포 도구는 사용하지 않습니다.
